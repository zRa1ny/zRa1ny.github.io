var Car = (function () {
    ...
    car.prototype.creatCar = function () {
        this.$car = $('<div style="position:absolute;background:url(' + this.config.src + ') no-repeat;background-size:100% 100%;left:' + (pos[0].x - road - this.config.width / 2) * pxToVh + 'vh;top: (pos[0].y - road - this.config.height / 2) * pxToVh ;width:' + this.config.width * pxToVh + 'vh;height:' + this.config.height * pxToVh + 'vh;transition:all 2s linear;transform:rotateZ(0deg) rotateY(-180deg)"></div>');
...
var playmusic = {
    changeClass: function (target, id) {
        var className = $(target).attr('class'), ids = document.getElementById(id);
        (className == 'play') ? $(target).removeClass('play').addClass('pause') : $(target).removeClass('pause').addClass('play'); (className == 'pause') ? ids.pause() : ids.play()
    },
    play: function () { document.getElementById('media').play() }
};
