window.onload=function () {
	...
	postBo(obj1)
	var obj2=write("组织冒泡和组织默认","今天群里有一个同学发了一个问题，问题起因是他定义了一个drag函数，然后函数内部使用了e进行组织默认实践，结果在360浏览器正常运行但是火狐却报错，e不存在，第一时间有点懵，组织冒泡和组织默认一般都用，那里需要那里用，一般式不会出现这种问题。因为新手才有这种操作，但是引发我兴趣，经过一番测试，我发现在360里面时间默认进行了传递，所以在360可以正常运行，但是在火狐里面没有进行传递。然后我传递参数为事件对象进去，执行成功，然后引发了我对几个浏览器不同引擎的猜测和测验，因为个人博客没有上传服务器，仅作记录，后面会在我博客园微博放出源码和测试结果。")
	postBo(obj2)
